...
public class InMemoryStore : ISharpInspectStore
{
    ...
    public InMemoryStore()
        : this(DefaultNetworkCapacity, DefaultConsoleCapacity, DefaultPerformanceCapacity, null)
    {
    }

    /// <summary>
    ///     기본 용량과 EventBus로 새 인메모리 스토어를 생성합니다.
    /// </summary>
    public InMemoryStore(EventBus eventBus)
        : this(DefaultNetworkCapacity, DefaultConsoleCapacity, DefaultPerformanceCapacity, eventBus)
    {
    }
    ...
    public InMemoryStore(int networkCapacity, int consoleCapacity)
        : this(networkCapacity, consoleCapacity, DefaultPerformanceCapacity, null)
    {
    }

    /// <summary>
    ///     지정된 용량과 EventBus로 새 인메모리 스토어를 생성합니다.
    /// </summary>
    public InMemoryStore(int networkCapacity, int consoleCapacity, EventBus eventBus)
        : this(networkCapacity, consoleCapacity, DefaultPerformanceCapacity, eventBus)
    {
    }
    ...
    public InMemoryStore(int networkCapacity, int consoleCapacity, int performanceCapacity)
        : this(networkCapacity, consoleCapacity, performanceCapacity, null)
    {
    }

    /// <summary>
    ///     모든 엔트리 타입에 대해 지정된 용량과 EventBus로 새 인메모리 스토어를 생성합니다.
    /// </summary>
    public InMemoryStore(int networkCapacity, int consoleCapacity, int performanceCapacity, EventBus eventBus)
    {
    ...
    public NetworkEntry GetNetworkEntry(string id)
    {
        ...
        {
            if (_networkIndex.TryGetValue(id, out var entry)) return entry;
            return null;
        }
    }
    ...
    public void AddConsoleEntry(ConsoleEntry entry)
    {
        ...
        PublishEvent(new ConsoleEntryEvent(entry));
    }
    ...
    public void AddNetworkEntry(NetworkEntry entry)
    {
        ...
        PublishEvent(new NetworkEntryEvent(entry));
    }
    ...
    public void AddPerformanceEntry(PerformanceEntry entry)
    {
        ...
        PublishEvent(new PerformanceEntryEvent(entry));
    }
    ...
    public void SetApplicationInfo(ApplicationInfo info)
    {
        ...
        if (info != null)
        {
            PublishEvent(new ApplicationInfoEvent(info));
        }
    }
