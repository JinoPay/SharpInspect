...
public static class SharpInspectDevTools
{
    ...
    public static void Initialize(SharpInspectOptions options)
    {
        lock (_lock)
        {
            ...
            // EventBus를 Store에 전달하여 엔트리 추가 시 자동으로 이벤트 발행
            ...
            HttpWebRequestInterceptor.Initialize(_store, Options);

            // 콘솔 후킹 초기화
            if (Options.EnableConsoleCapture)
            {
                _consoleHook = new ConsoleHook(_store, Options);
                _traceHook = new TraceHook(_store, Options);
            }

            // 성능 캡처 초기화
            if (Options.EnablePerformanceCapture) _performanceInterceptor = new PerformanceInterceptor(_store, Options);
    ...
    public static HttpClient CreateHttpClient()
    {
        ...
        var handler = new SharpInspectHandler(_store, Options);
    ...
    public static DelegatingHandler CreateHandler()
    {
        ...
        return new SharpInspectHandler(_store, Options);
    }
    ...
    public static DelegatingHandler CreateHandler(HttpMessageHandler innerHandler)
    {
        ...
        return new SharpInspectHandler(_store, Options, innerHandler);
    }
    ...
    /// </summary>
    private class PassThroughHandler(HttpMessageHandler innerHandler) : DelegatingHandler(innerHandler)
    {
        public PassThroughHandler() : this(new HttpClientHandler())
        {
        }
    }
}
